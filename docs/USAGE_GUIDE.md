📖 详细使用指南 - data_dealer_auto
================================================================================

完整的Pick/Place数据集自动化处理指南

📚 目录
─────────────────────────────────────────────────────────────────────────────
1. 基本概念
2. 快速开始
3. 详细参数说明
4. 使用案例
5. 输出说明
6. 常见问题
7. 故障排除
8. 扩展和自定义

1️⃣ 基本概念
─────────────────────────────────────────────────────────────────────────────

Pick/Place检测原理
  机器人动作向量有7个维度：
  [x平移, y平移, z平移, α旋转, β旋转, γ旋转, 夹爪状态]
                                              ↑ 第7维

  夹爪状态范围：-1.0（打开）到 1.0（关闭）
  
  关键动作：
    -1.0 → 1.0（从打开到关闭）= PICK 操作
     1.0 → -1.0（从关闭到打开）= PLACE 操作

帧范围提取
  关键帧索引：i
  提取范围：[max(0, i-30), min(总帧数, i+31)]
  总帧数：60帧（或更少，靠近边界时）
  
  目的：捕捉完整的操作过程
    ├─ 前30帧：接近目标、定位
    ├─ 当前帧：关键动作（夹爪改变）
    └─ 后30帧：执行操作、返回

任务描述生成
  原始任务：从数据集元数据获取
  新任务：根据操作类型自动生成
    例：原始任务 "put both moka pots on the stove"
        pick操作 → "pick up the moka pot"
        place操作 → "put the moka pot on the stove"

2️⃣ 快速开始
─────────────────────────────────────────────────────────────────────────────

🌟 推荐：使用Azure OpenAI GPT-5获得最佳任务描述质量

如果你有Azure OpenAI企业账号，强烈推荐使用GPT-4o：
  
  rm -rf /inspire/hdd/project/robot-decision/laijunxi-CZXS25230141/datasets/HuggingFaceVLA_cus/datasets_cut && time python auto_cut_dataset.py \
    --dataset-path /inspire/hdd/project/robot-decision/public/datasets/HuggingFaceVLA_cus/libero \
    --output-dir /inspire/hdd/project/robot-decision/laijunxi-CZXS25230141/datasets/HuggingFaceVLA_cus/datasets_cut \
    --before-frames 15 \
    --after-frames 10 \
    --llm-provider gpt \
    --llm-api-key 5ffef770a5b148c5920b7b16329e30fa \
    --llm-api-base https://gpt.yunstorm.com/ \
    --llm-api-version 2025-01-01-preview \
    --llm-model gpt-4o 2>&1 | tee data_cut.log

  rm -rf /inspire/ssd/project/robot-decision/laijunxi-CZXS25230141/data_dealer_auto/datasets_cut && time python auto_cut_dataset.py \
    --dataset-path /inspire/hdd/project/robot-decision/public/datasets/HuggingFaceVLA_cus/libero \
    --output-dir /inspire/ssd/project/robot-decision/laijunxi-CZXS25230141/data_dealer_auto/datasets_cut \
    --before-frames 15 \
    --after-frames 10 \
    --llm-provider gpt \
    --end-idx 1000 \
    --llm-api-key 5ffef770a5b148c5920b7b16329e30fa \
    --batch-size 10 \
    --llm-api-base https://gpt.yunstorm.com/ \
    --llm-api-version 2025-01-01-preview \
    --llm-model gpt-4o 2>&1 | tee data_cut.log

  rm -rf /inspire/ssd/project/robot-decision/laijunxi-CZXS25230141/data_dealer_auto/datasets_cut_placeholder && time python auto_cut_dataset.py \
    --dataset-path /inspire/hdd/project/robot-decision/public/datasets/HuggingFaceVLA_cus/libero \
    --output-dir /inspire/ssd/project/robot-decision/laijunxi-CZXS25230141/data_dealer_auto/datasets_cut_placeholder \
    --load-ranges /inspire/ssd/project/robot-decision/laijunxi-CZXS25230141/data_dealer_auto/datasets_cut/frame_ranges_info.json \
    --batch-size 10 \
    --insert-placeholders 

  优势：
    ✓ GPT-5视觉理解能力，精准识别操作对象
    ✓ 分析多帧图像（首帧、关键帧、尾帧）
    ✓ 支持双摄像头视角分析
    ✓ 生成自然、准确的任务描述
  
  测试API可用性：python test_azure_gpt.py

─────────────────────────────────────────────────────────────────────────────

最简单的3步（本地方式，无需API）：

第1步：进入目录
  cd /home/dongyingyibadao/data_dealer_auto

第2步：分析数据（仅检测，不转换）
  python auto_cut_dataset.py --end-idx 10000 --skip-cutting
  
  ✓ 快速生成分析报告
  ✓ 不占用大量磁盘空间
  ✓ 输出：cut_dataset/frame_ranges_info.json

第3步：执行完整处理
  python auto_cut_dataset.py --end-idx 10000 --max-episodes 100
  
  ✓ 转换为LeRobot格式
  ✓ 输出：cut_dataset/meta 和 cut_dataset/data
  ✓ 数据按原始episode归类：cut_dataset/data/episode_{id}/segment_{id}.parquet

3️⃣ 详细参数说明
─────────────────────────────────────────────────────────────────────────────

python auto_cut_dataset.py [OPTIONS]

--dataset-path PATH
  说明：输入数据集路径
  默认：/home/dongyingyibadao/HuggingFaceVLA_cus/libero
  示例：--dataset-path /path/to/dataset

--output-dir PATH
  说明：输出目录
  默认：./cut_dataset（当前目录下）
  示例：--output-dir /tmp/output_data

--start-idx INDEX
  说明：开始处理的帧索引
  默认：0
  示例：--start-idx 1000

--end-idx INDEX
  说明：结束处理的帧索引
  默认：None（处理所有数据）
  用途：用于测试和控制数据量
  示例：--end-idx 50000
  注意：不指定时将处理完整数据集

--before-frames N
  说明：关键帧前取的帧数
  默认：30
  用途：调整动作前摇长度
  示例：--before-frames 40

--after-frames N
  说明：关键帧后取的帧数
  默认：30
  用途：调整动作后摇长度
  示例：--after-frames 20

--max-episodes N
  说明：最多保存的episode数量
  默认：None（全部保存）
  用途：限制输出文件大小
  示例：--max-episodes 100

--llm-provider PROVIDER
  说明：任务描述生成使用的LLM
  选项：local, qwen, deepseek, gpt
  默认：local
  注意：使用gpt需要提供API Key，且支持VLM（视觉语言模型）
  示例：--llm-provider gpt

--llm-api-key KEY
  说明：LLM API密钥
  用途：用于qwen, deepseek, gpt
  示例：--llm-api-key sk-xxxxxx

--llm-api-base URL
  说明：LLM API基础URL
  用途：用于自定义API端点或Azure OpenAI
  默认：根据provider自动选择
  示例：--llm-api-base https://gpt.yunstorm.com/

--llm-api-version VERSION
  说明：LLM API版本
  用途：仅用于Azure OpenAI服务
  示例：--llm-api-version 2025-01-01-preview

--llm-model MODEL
  说明：指定LLM模型名称
  用途：覆盖默认模型
  支持的模型（需要Vision能力）：
    - gpt-5（最新，推荐）
    - gpt-4.1, gpt-4.1-mini
    - gpt-4o, gpt-4o-mini
    - gpt-4-turbo
  示例：--llm-model gpt-5

--llm-fast-mode
  说明：GPT快速模式（仅上传2帧图像）
  类型：布尔标志（无参数）
  用途：加快处理速度，降低API调用成本
  说明：
    - 精细模式（默认）：上传6帧（cam1和cam2各3帧）
    - 快速模式：仅上传2帧（cam1的首尾帧）
    - 速度提升：约3倍
  示例：--llm-fast-mode

--checkpoint-interval N
  说明：checkpoint保存间隔（每处理N个任务保存一次）
  默认：10
  用途：防止长时间运行时数据丢失
  示例：--checkpoint-interval 20
  输出：./cut_dataset/checkpoints/checkpoint_*.json

--resume-from FILE
  说明：从checkpoint文件恢复运行
  用途：中断后继续处理，不会重复已完成的任务
  示例：--resume-from ./cut_dataset/checkpoints/checkpoint_latest.json
  注意：需要使用与中断前相同的参数

--llm-provider {local, qwen, deepseek, gpt}
  说明：任务描述生成方法
  选项：
    local     - 无需API，使用规则生成（默认）
    qwen      - 调用阿里Qwen模型（需要API密钥）
    deepseek  - 调用Deepseek模型（需要API密钥）
  示例：--llm-provider qwen

--llm-api-key KEY
  说明：LLM API密钥
  仅在使用qwen或deepseek时需要
  示例：--llm-api-key sk-abc123def456

--skip-cutting
  说明：跳过数据集转换，仅执行分析
  类型：布尔标志（无参数）
  用途：快速分析，查看结果再决定是否转换
  示例：--skip-cutting

--load-ranges FILE
  说明：加载之前保存的分析结果
  用途：避免重复分析，直接进行数据转换
  示例：--load-ranges ./cut_dataset/frame_ranges_info.json
  注意：可以与--insert-placeholders同时使用

--insert-placeholders
  说明：在同一chunk的不同segments之间物理插入placeholder（方案3）
  类型：布尔标志（无参数）
  用途：生成包含placeholder的数据集，placeholder永久存在于磁盘
  特点：
    - Placeholder物理写入磁盘
    - Meta信息自动包含正确的索引
    - 可与--load-ranges配合使用
    - 适合数据集分发和归档
  示例：--insert-placeholders
  详见：docs/PLACEHOLDER_GENERATION_GUIDE.md

--placeholder-action-value VALUE
  说明：placeholder的action值
  默认：-999.0
  用途：自定义placeholder的识别值
  示例：--placeholder-action-value -1000.0
  注意：需要与--insert-placeholders一起使用

4️⃣ 使用案例
─────────────────────────────────────────────────────────────────────────────

📌 案例1：快速测试（仅分析，不转换）
  
  python auto_cut_dataset.py --end-idx 10000 --skip-cutting
  
  优点：快速、不占用磁盘空间
  输出：cut_dataset/frame_ranges_info.json
  用途：查看数据统计，决定是否继续处理

📌 案例2：完整处理（分析+转换，100个episode）
  
  python auto_cut_dataset.py --end-idx 10000 --max-episodes 100
  
  输出：
    ├─ cut_dataset/meta/episodes/chunk-000/file-000.parquet (2.5 MB)
    ├─ cut_dataset/meta/tasks.parquet (50 KB)
    ├─ cut_dataset/data/chunk-000/file-*.parquet (500+ MB)
    └─ cut_dataset/frame_ranges_info.json (1.5 MB)
  总大小：2-3 GB
  用途：生成完整的、可用的数据集

📌 案例3：处理完整数据集（所有数据）🎯
  
  # 不指定 --end-idx，自动处理所有数据
  python auto_cut_dataset.py \
    --llm-provider local \
    --max-episodes 1000
  
  说明：不指定 --end-idx 时，将处理完整数据集
  警告：输出非常大（可能 50+ GB），请确保：
    1. 磁盘空间充足（建议 100GB+）
    2. 预留足够时间（可能需要数小时）
  用途：生成完整的生产级训练数据集

📌 案例4：处理指定范围的数据（测试用）
  
  python auto_cut_dataset.py --end-idx 100000 --max-episodes 500
  
  说明：指定 --end-idx 可以控制处理范围
  用途：测试流程或生成中等规模数据集

📌 案例5：使用Azure OpenAI GPT-5生成精准任务描述（推荐）⭐
  
  # 精细模式（默认）- 上传6帧图像，质量最高
  python auto_cut_dataset.py \
    --end-idx 500 \
    --before-frames 15 \
    --after-frames 10 \
    --llm-provider gpt \
    --llm-api-key 5ffef770a5b148c5920b7b16329e30fa \
    --llm-api-base https://gpt.yunstorm.com/ \
    --llm-api-version 2025-01-01-preview \
    --llm-model gpt-5 \
    --save-mode image
  
  # 快速模式 - 仅上传2帧图像，速度提升3倍 🚀
  python auto_cut_dataset.py \
    --end-idx 500 \
    --before-frames 15 \
    --after-frames 10 \
    --llm-provider gpt \
    --llm-api-key 5ffef770a5b148c5920b7b16329e30fa \
    --llm-api-base https://gpt.yunstorm.com/ \
    --llm-api-version 2025-01-01-preview \
    --llm-model gpt-5 \
    --llm-fast-mode \
    --save-mode image
  
  需要：Azure OpenAI API密钥（企业账号）
  
  模式对比：
    精细模式（默认）：
      ✓ 上传6帧图像（cam1和cam2各3帧：首帧、关键帧、尾帧）
      ✓ 最高质量，最准确的物体识别
      ✓ 适合复杂场景和多物体任务
      ✗ 处理较慢，API成本较高
    
    快速模式（--llm-fast-mode）：
      ✓ 仅上传2帧图像（cam1的首帧和尾帧）
      ✓ 速度提升约3倍，API成本降低约66%
      ✓ 适合简单场景和单物体任务
      ~ 质量略有下降，但通常足够准确
  
  其他优点：
    ✓ 使用最新GPT-5模型（gpt-5-2025-08-07）
    ✓ 支持多模态视觉理解
    ✓ 精准识别操作对象和位置
    ✓ 生成自然、准确的任务描述
  耗时：快速模式比精细模式快3倍，但仍比本地方法慢

📌 案例6：使用Qwen生成更精准的任务描述
  
  python auto_cut_dataset.py \
    --llm-provider qwen \
    --llm-api-key sk-xxx \
    --end-idx 10000 \
    --max-episodes 100
  
  需要：Qwen API密钥
  优点：生成的任务描述更自然、准确
  耗时：比本地方法慢5-10倍

📌 案例7：重复使用分析结果
  
  # 第一次运行（仅分析）
  python auto_cut_dataset.py --end-idx 10000 --skip-cutting
  
  # 第二次运行（直接转换，使用保存的分析结果）
  python auto_cut_dataset.py \
    --load-ranges cut_dataset/frame_ranges_info.json \
    --max-episodes 200
  
  优点：避免重复分析，节省时间
  用途：调整episode数量或其他参数

📌 案例7b：使用已有分析结果 + 插入Placeholder（方案3）⚡
  
  # 第一次运行（仅分析并保存）
  python auto_cut_dataset.py --end-idx 10000 --skip-cutting
  
  # 第二次运行（加载分析结果 + 物理插入placeholder）
  python auto_cut_dataset.py \
    --load-ranges cut_dataset/frame_ranges_info.json \
    --max-episodes 200 \
    --insert-placeholders \
    --placeholder-action-value -999.0
  
  优点：
    ✓ 跳过分析和LLM步骤，直接转换
    ✓ 在转换时插入物理placeholder
    ✓ 可以对同一分析结果生成不同版本（有/无placeholder）
  用途：
    - 快速生成多个数据集版本
    - 测试placeholder对训练的影响
    - 数据集发布前的最终处理

📌 案例8：指定输出目录
  
  python auto_cut_dataset.py \
    --output-dir /data/my_dataset \
    --end-idx 10000 \
    --max-episodes 100
  
  输出到：/data/my_dataset/
  用途：组织多个数据集或使用外部存储

📌 案例8b：生成带Placeholder的数据集（方案3 - 物理写入）⚡
  
  # 直接生成带placeholder的数据集
  python auto_cut_dataset.py \
    --dataset-path /path/to/dataset \
    --output-dir /data/dataset_with_ph \
    --end-idx 10000 \
    --max-episodes 100 \
    --insert-placeholders \
    --placeholder-action-value -999.0
  
  # 或使用已有的分析结果
  python auto_cut_dataset.py \
    --load-ranges cut_dataset/frame_ranges_info.json \
    --output-dir /data/dataset_with_ph \
    --insert-placeholders
  
  Placeholder特性：
    ✓ 在同一chunk的不同segments之间物理插入
    ✓ Action值全为-999.0（可自定义）
    ✓ Meta信息自动包含正确的索引
    ✓ 可用标准LeRobotDataset直接加载
  
  用途：
    - 数据集分发和归档
    - 训练时需要明确的轨迹分割标记
    - 与方案1（运行时动态）对比实验
  
  详见：docs/LOAD_RANGES_GUIDE.md（Placeholder章节）

📌 案例9：大数据集处理 + Checkpoint保护（推荐）🛡️
  
  # 处理完整数据集（270k帧），带checkpoint保护
  python auto_cut_dataset.py \
    --llm-provider gpt \
    --llm-api-key 5ffef770a5b148c5920b7b16329e30fa \
    --llm-api-base https://gpt.yunstorm.com/ \
    --llm-api-version 2025-01-01-preview \
    --llm-model gpt-4o \
    --checkpoint-interval 10
  
  # 如果中断，从checkpoint恢复
  python auto_cut_dataset.py \
    --llm-provider gpt \
    --llm-api-key 5ffef770a5b148c5920b7b16329e30fa \
    --llm-api-base https://gpt.yunstorm.com/ \
    --llm-api-version 2025-01-01-preview \
    --llm-model gpt-4o \
    --resume-from ./cut_dataset/checkpoints/checkpoint_latest.json
  
  # 或使用交互式脚本
  bash scripts/run_with_checkpoint.sh
  
  优势：
    ✓ 自动保存进度（每10个任务）
    ✓ 错误时立即保存checkpoint
    ✓ 支持断点续传，不丢失进度
    ✓ 处理大规模数据集必备
  
  Checkpoint文件位置：
    - checkpoint_latest.json: 最新状态（用于恢复）
    - checkpoint_progress_*.json: 定期保存
    - checkpoint_error_*.json: 错误时保存
    - checkpoint_final.json: 完成时保存
  
  详细文档：docs/CHECKPOINT_GUIDE.md

5️⃣ 输出说明
─────────────────────────────────────────────────────────────────────────────

执行完成后，输出目录结构如下：

cut_dataset/
├── meta/
│   ├── episodes/
│   │   └── chunk-000/
│   │       └── file-000.parquet          ← Episodes元数据
│   │           包含：episode_index, tasks, dataset_from_index,
│   │                 dataset_to_index, length, action_type等
│   ├── tasks.parquet                     ← 任务列表
│   │   包含：所有唯一的任务描述
│   ├── info.json                         ← 数据集信息
│   │   {
│   │     "total_episodes": 100,
│   │     "total_frames": 6000,
│   │     "total_tasks": 40,
│   │     "created_at": "2025-11-24T...",
│   │     "robot_type": "Panda 7-DOF",
│   │     "sampling_frequency": 10
│   │   }
│   └── stats.json                        ← 统计信息
│       {
│         "average_frames_per_episode": 60,
│         "min_frames_per_episode": 55,
│         "max_frames_per_episode": 65
│       }
├── data/
│   └── chunk-000/
│       ├── file-000.parquet              ← 帧数据（500 MB）
│       ├── file-001.parquet
│       └── ...
└── frame_ranges_info.json                ← 分析报告
    {
      "total_ranges": 100,
      "pick_count": 45,
      "place_count": 55,
      "frame_ranges": [
        {
          "id": 0,
          "keyframe_index": 100727,
          "action_type": "place",
          "frame_start": 100697,
          "frame_end": 100757,
          "num_frames": 60,
          "original_task": "put moka pot on stove",
          "new_task": "put the moka pot on the stove",
          "episode_index": 376
        },
        ...
      ]
    }

帧数据文件内容：
  每个parquet文件包含多帧数据，每帧包括：
    - observation.images.image (3, 256, 256)    ← RGB图像
    - observation.images.image2 (3, 256, 256)   ← 第二个RGB图像
    - observation.state (8,)                     ← 关节状态
    - action (7,)                                ← 动作向量
    - timestamp (float)                          ← 时间戳

6️⃣ 常见问题
─────────────────────────────────────────────────────────────────────────────

Q: 为什么检测不到关键帧？
A: 可能原因：
   1. 夹爪状态变化不明显
   2. 修改gripper_detector.py中的threshold参数
   3. 检查原始数据是否包含action信息

Q: 如何只处理特定类型的操作（pick或place）？
A: 方案：
   1. 运行完整分析获得frame_ranges_info.json
   2. 手动编辑JSON文件，删除不需要的操作
   3. 重新运行数据转换

Q: 磁盘空间不足怎么办？
A: 解决方案：
   1. 减少--end-idx范围
   2. 降低--max-episodes数量
   3. 删除之前生成的cut_dataset并重新生成
   4. 使用外部存储：--output-dir /external/disk

Q: 如何提升任务描述质量？
A: 方案：
   1. 🌟 使用Azure OpenAI GPT-5（推荐）- 支持视觉理解，质量最高
      python auto_cut_dataset.py \
        --llm-provider gpt \
        --llm-api-key YOUR_KEY \
        --llm-api-base https://gpt.yunstorm.com/ \
        --llm-api-version 2025-01-01-preview \
        --llm-model gpt-5
   2. 使用Qwen或Deepseek LLM（需要API）
   3. 手动编辑frame_ranges_info.json中的new_task
   4. 在Web界面中进行修正

Q: Azure OpenAI支持哪些模型？
A: 经测试，以下模型可用（纯文本）：
   ✅ gpt-5 (gpt-5-2025-08-07) - 纯文本可用
   ✅ gpt-4.1 (gpt-4.1-2025-04-14)
   ✅ gpt-4.1-mini (gpt-4.1-mini-2025-04-14)
   ✅ gpt-4o (gpt-4o-2024-11-20)
   ✅ gpt-4o-mini (gpt-4o-mini-2024-07-18)
   ✅ gpt-4-turbo (实际使用gpt-4.1)
   ❌ gpt-4, gpt-3.5-turbo（不支持）
   
   ⚠️  注意：Vision（图像）功能在该端点不可用
   - 纯文本调用：✅ 正常工作
   - 图像调用：❌ 返回 401 Unauthorized
   - 原因：Vision 功能未启用或需要额外权限
   
   测试工具：python test_azure_gpt.py

Q: GPT Vision 功能返回 401 错误怎么办？
A: 这是已知问题，该 Azure OpenAI 端点不支持 Vision 功能：
   错误信息：Response status code does not indicate success: 401 (Unauthorized)
   
   解决方案：
   1. 联系管理员启用 Vision 功能或提供 Vision API Key
   2. 使用本地模式：--llm-provider local（无需API）
   3. 使用其他 LLM：--llm-provider qwen 或 deepseek
   
   临时命令（本地模式）：
   python auto_cut_dataset.py \
     --end-idx 500 \
     --llm-provider local \
     --save-mode image

Q: 转换太慢怎么办？
A: 优化建议：
   1. 减少--end-idx范围，分批处理
   2. 使用GPU加速（修改代码中的device）
   3. 在SSD上运行
   4. 增加帧批处理大小
   5. 使用GPT快速模式：--llm-fast-mode（速度提升3倍）

Q: 处理大数据集时如何防止数据丢失？
A: 使用checkpoint功能：
   1. 启用自动checkpoint：--checkpoint-interval 10
   2. 程序会每10个任务保存一次进度
   3. 错误时自动保存checkpoint
   4. 中断后使用 --resume-from 恢复
   
   示例：
   # 初始运行
   python auto_cut_dataset.py [参数] --checkpoint-interval 10
   
   # 如果中断，恢复运行
   python auto_cut_dataset.py [相同参数] \
     --resume-from ./cut_dataset/checkpoints/checkpoint_latest.json
   
   详见：docs/CHECKPOINT_GUIDE.md

Q: 如何查看checkpoint状态？
A: 查看checkpoint文件内容：
   cat ./cut_dataset/checkpoints/checkpoint_latest.json | python -m json.tool
   
   包含信息：
   - completed_tasks: 已完成的任务列表
   - completed_ranges: 已处理的索引范围
   - timestamp: 保存时间
   - total_completed: 完成总数

Q: checkpoint文件在哪里？
A: 默认位置：./cut_dataset/checkpoints/
   文件类型：
   - checkpoint_latest.json: 最新状态（用于恢复）
   - checkpoint_progress_*.json: 定期保存的进度
   - checkpoint_error_*.json: 错误时的状态
   - checkpoint_final.json: 完成时的最终状态

Q: 什么是Placeholder？何时使用？
A: Placeholder是在同一chunk的不同segments之间插入的特殊标记帧：
   
   两种方案：
   1. 方案1（运行时动态）：使用LeRobotDatasetWithPlaceholder包装器
      - 不修改原始数据
      - 适合训练使用
      - 详见：docs/LEROBOT_DATASET_PLACEHOLDER_USAGE.md
   
   2. 方案3（生成时物理写入）：使用--insert-placeholders参数
      - 物理写入磁盘
      - Meta信息自动正确
      - 适合数据集分发
      - 详见：docs/LOAD_RANGES_GUIDE.md
   
   使用场景：
   - 标记轨迹的不连续边界
   - 训练时分割子轨迹
   - 避免模型学习非连续动作

Q: 如何生成带Placeholder的数据集？
A: 使用方案3（物理写入）：
   python auto_cut_dataset.py \
     --insert-placeholders \
     --placeholder-action-value -999.0 \
     [其他参数...]
   
   可与--load-ranges配合使用：
   python auto_cut_dataset.py \
     --load-ranges cut_dataset/frame_ranges_info.json \
     --insert-placeholders

Q: 训练时如何处理Placeholder？
A: 三种方式：
   1. 通过meta过滤：检查is_placeholder字段
   2. 通过action值：检测action是否全为-999.0
   3. 使用collate_fn：在DataLoader中过滤
   
   详细代码见：docs/LOAD_RANGES_GUIDE.md

7️⃣ 故障排除
─────────────────────────────────────────────────────────────────────────────

错误：ModuleNotFoundError: No module named 'lerobot'
解决：pip install lerobot

错误：CUDA out of memory
解决：设置CPU模式
  export CUDA_VISIBLE_DEVICES=0
  或修改代码中device设置

错误：FileNotFoundError: dataset not found
解决：检查数据集路径是否正确
  python auto_cut_dataset.py --dataset-path /correct/path

错误：内存占用过高
解决：
  1. 减少处理范围
  2. 降低episode限制
  3. 使用流式处理（修改代码）

8️⃣ 扩展和自定义
─────────────────────────────────────────────────────────────────────────────

自定义检测阈值
  编辑gripper_detector.py，修改第29行：
    self.threshold = 0.5  # 改为你需要的值
  
  阈值说明：
    < 0.3：敏感，容易误检
    0.5：默认，平衡
    > 0.8：保守，可能漏检

自定义帧范围（不是30帧）
  编辑auto_cut_dataset.py，修改第89行：
    ranges = detector.extract_frame_ranges(changes, context_frames=50)
  改为你需要的帧数

添加自定义LLM
  在task_description_generator.py中：
  1. 继承LLMProvider基类
  2. 实现generate_task_description()方法
  3. 在TaskDescriptionGenerator中注册

集成其他数据源
  修改auto_cut_dataset.py中的load_lerobot_dataset()
  支持其他数据格式

================================================================================
需要帮助？运行：python auto_cut_dataset.py --help
================================================================================
